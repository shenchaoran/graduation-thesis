...
\begin{enumerate}[(1)]
    ...
    \item \textbf{运行接口：}模型的数据读取方式各异，可以简单分为硬耦合方式、配置文件方式和命令行参数三种，每一类下又存在不同的具体标准，如图~\ref{fig:before-encap}，IBIS、Biome-BGC和LPJ的运行接口各不相同，其中IBIS和LPJ通过源代码硬耦合的方式，这种方式在更换数据时需要重新编译，及其不便；Biome-BGC通过将输入数据写在配置文件中，比硬耦合的方式更加灵活，但是不同模型的配置方式很难定义标准。
\end{enumerate}
